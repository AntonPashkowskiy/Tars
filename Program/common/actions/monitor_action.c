...
struct monitor_action parse_message(char* message)
{
    ...
    message_part = strtok(message, MESSAGE_PARTS_DELIMETER);
    action.id = message_part == NULL ? 0 : atoi(message_part);

    message_part = strtok(NULL, MESSAGE_PARTS_DELIMETER);
...
char* get_message(struct monitor_action action)
{
    char* message;
    char* id_part;
    char* modificators_part;
    int message_length;

    id_part = itoa(action.id, 10);
    modificators_part = itoa(action.modificators, 10);
